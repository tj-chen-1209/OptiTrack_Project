...
#Unpack Mocap Data Functions
    ...
    def __command_thread_function( self, in_socket, stop, gprint_level, thread_option):
        message_id_dict={}
        ...
        buffer_list_size = 4 
        buffer_list = [bytearray(recv_buffer_size) for _ in range(buffer_list_size)]
                ...
                if message_id == self.NAT_FRAMEOFDATA:
                    if print_level > 0:
                        if (message_id_dict[tmp_str] % print_level) == 0:
                            ...
                            print_level = 0
                message_id = self.__process_message( buffer_list[buffer_list_in_use_index] , print_level)
    ...
    def run( self, thread_option):
        ...
        self.command_thread = Thread( target = self.__command_thread_function, args = (self.command_socket, lambda : self.stop_threads, lambda : self.print_level, thread_option,))
        if thread_option == 'd':
